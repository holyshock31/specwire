# workflow Specification（delta：SpecWire 集成边界重置）
...
### Requirement: 两套事件模型分工明确，互不干扰

**Reason**: The old requirement treats `proposal-ready` Push Hook as a supported publication path and defines two competing publication models. v1 publication is retired.

**Migration**: Use `SpecWire 只处理发布与归档两个集成事件` and the Bridge event-routing contract. Push Hook is retained only for `archived` completion.

### Requirement: 发布者按次指定建卡状态与分配

**Reason**: Initial projection status and assignment are Bridge input semantics, not an end-to-end workflow or Agent responsibility.

**Migration**: Use the Bridge contract for `SpecWire-Status` and `SpecWire-Assignee`; client Skills may choose whether to include those fields.

### Requirement: 归档在 main 上执行，主规格文档由 archive 自动合并

**Reason**: The old requirement couples SpecWire to the local `openspec archive` command and to how main specifications are merged. Those are repository workflow operations owned by the client Skill layer.

**Migration**: Keep only the externally observable `archived` event and projection-completion behavior in SpecWire specs; document the local archive command in SpecWire Skills.

### Requirement: change 文档可在任意分支开发，发布收敛到 main

**Reason**: Branch and change-document movement is repository workflow, not an integration behavior owned by SpecWire.

**Migration**: Keep branch/repository procedures in SpecWire Skills and retain only the publication metadata and event contract in SpecWire specs.
