# bridge Specification（delta：v2 契约重置）
...
### Requirement: 多 change 一次发布各自建卡

**Reason**: This requirement describes the v1 model in which one Push Hook carries multiple `proposal-ready` commit trailers. New publication is now one GitLab `change` Issue per immutable publication.

**Migration**: Clients publish one Issue per change. A Push Hook may still carry multiple `archived` completion events, but none of them creates a new execution projection.
